#!/usr/bin/env python3
"""
Pre-compute per-event active station availability for the Cascadia benchmark.

Sibling of build_station_availability.py — same probing logic, but Cascadia
is a region-scale benchmark (not a benchmark.config.CASE_STUDIES entry) with
its own catalog format: data/cascadia/reference/cascadia_test_catalog.csv,
a plain USGS-download CSV (id/time/latitude/longitude/...) rather than the
tab-separated bEPIC_testing_catalog.txt or case-study *_catalog.parquet
formats the main script handles.

For each event in the Cascadia benchmark catalog:
...
Output
------
data/cascadia/reference/station_availability_cache.parquet
...
-----
    ...
    python preparation_scripts/build_station_availability_cascadia.py
...
RUN_DIR      = HERE / 'data' / 'cascadia' / 'run_files'
CATALOG_PATH = HERE / 'data' / 'cascadia' / 'reference' / 'cascadia_test_catalog.csv'
OUTPUT_PATH  = HERE / 'data' / 'cascadia' / 'reference' / 'station_availability_cache.parquet'

# ShakeAlert contributing networks (western US, incl. Pacific NW)
...
def load_cascadia_catalog(path):
    """Return dict {event_id (str): (lat, lon, UTCDateTime)} from the Cascadia
    reference catalog CSV (USGS download format: id/time/latitude/longitude/...)."""
    df = pd.read_csv(path)
...
def main():
    ...
    # Get catalog of events - need station availability per event
    catalog   = load_cascadia_catalog(CATALOG_PATH)
