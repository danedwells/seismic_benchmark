"""
plot_scripts/large_events_plot_comparison.py — comparison figures for the
data_examination_scripts/california_large_events.py benchmark (California
events restricted to usgs_mag >= MIN_MAG).

Fork of ETAS_plot_comparison.py, scoped to the three priors
california_large_events.py runs — KDE_Seismicity, Uniform, and one dynamic
ETAS config — over the magnitude-filtered event set that script produces.
There is no case-study mode (california_large_events.py only runs the main
benchmark catalog) and no mixed-prior or per-ETAS-config comparison
(california_large_events.py runs exactly one ETAS config), so those parts of
ETAS_plot_comparison.py are dropped here.

Set MIN_MAG to match the threshold california_large_events.py was run with —
results are read from results/california/M{MIN_MAG}/output/..., mirroring
that script's output directory tree.

Requires benchmark CSVs generated by the updated runner that saves metrics
(map_err_km, coverage, posterior_confidence_level) at every trigger version.
...
# ---------------------------------------------------------------------------
# Magnitude filter — must match california_large_events.py's MIN_MAG
# ---------------------------------------------------------------------------
MIN_MAG    = 5.0
MAG_FILTER = f"M{MIN_MAG:g}"

PLOT_TITLE_SUFFIX = f'large events (M≥{MIN_MAG:g})'

# ---------------------------------------------------------------------------
# Resolve output paths — mirrors california_large_events.py's directory tree
# ---------------------------------------------------------------------------
MAX_TRIGS = 10  # matches config.BENCHMARK_PARAMS['max_trigs'] used by california_large_events.py
...
FIGURES_DIR        = os.path.join(MAG_DIR, 'figures', 'etas_comparison')

os.makedirs(FIGURES_DIR, exist_ok=True)

# ---------------------------------------------------------------------------
# Prior specs — the three priors california_large_events.py runs
# ---------------------------------------------------------------------------
PRIOR_SPECS = [
    {'name':  name,
...
# PRIOR_SPECS: 'KDE_Seismicity', 'Uniform', or 'ETAS'.
REFERENCE_PRIOR = 'KDE_Seismicity'
#REFERENCE_PRIOR = 'Uniform'
#REFERENCE_PRIOR = 'ETAS'
...
# zip (not enumerate + PRIOR_SPECS[i]) so this doesn't IndexError since only
# 3 PRIOR_SPECS are active against the fixed 2x3 (6-axes) layout — unused
# axes are hidden below rather than assumed to exist 1:1 with axes.
for ax, spec in zip(axes, PRIOR_SPECS):
    ...
    # Plot stats — None when spec['csv'] hasn't been run yet; skip rather than crash.
...
fig.suptitle(f"Figure 8: {PLOT_TITLE_SUFFIX}  Number of triggers: {trigger_number}",fontsize=16)
...
# %%
# ---------------------------------------------------------------------------
# Figure 9: spatial map of location errors — one panel per prior
# ---------------------------------------------------------------------------
...
for ax, spec in zip(_axes10, PRIOR_SPECS):
...
for ax in _axes10[len(PRIOR_SPECS):]:
    ax.set_visible(False)

_y_max10 = max(ax.get_ylim()[1] for ax in _axes10[:len(PRIOR_SPECS)])
for ax in _axes10[:len(PRIOR_SPECS)]:
...
fig10.suptitle(
    f'Figure 11: MTJ region  |  {PLOT_TITLE_SUFFIX}  Number of triggers: {_trigger_number}',
    fontsize=16)
plt.show()
...
# ---------------------------------------------------------------------------
# Covers every entry in PRIOR_SPECS (KDE_Seismicity, Uniform, ETAS).
# plot_qq_calibration()/plot_qq_calibration_prior() normally derive each CSV
# path from output_dir + prior_name.lower(), which can't reach the ETAS
# results (they live under OUTPUT_DIR_DYNAMIC, not OUTPUT_DIR_STATIC) — so
# pass csv_paths (built from PRIOR_SPECS, which already has the correct path
# per entry) to override that for every name it covers.
...
# %%
# ---------------------------------------------------------------------------
# Figure 18: Spatial map of large-error events — one panel per prior
# ---------------------------------------------------------------------------
...
# keeps lat/lon — pull mag/depth from `_raw`, the same source object that
# built it).
# ---------------------------------------------------------------------------
if ref_catalog is not None:
...
# %%
# ---------------------------------------------------------------------------
# Figure 19: Magnitude vs location error — large-error events, one panel per prior
# ---------------------------------------------------------------------------
...
# %%
# ---------------------------------------------------------------------------
# Figure 20: Depth vs location error — large-error events, one panel per prior
# ---------------------------------------------------------------------------
