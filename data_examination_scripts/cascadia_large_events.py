#%%
# =============================================================================
# cascadia_large_events.py  —  bEPIC prior benchmark restricted to large events
#
# One-off script: copies the logic of time_independent_scripts/run_cascadia.py
# (KDE_Seismicity, Uniform) and time_dependent_scripts/run_cascadia.py (ETAS),
# but filters the Cascadia event list down to events with usgs_mag >= MIN_MAG
# and writes results under results/cascadia/{MAG_FILTER}/ instead of the main
# benchmark output tree.
#
# Prerequisites: preparation_scripts/build_priors.py and
# time_dependent_scripts/build_initial_prior_cascadia.py must already have been run.
# =============================================================================
import os
...
import benchmark.config_cascadia as config
...
MAG_DIR            = os.path.join(PROJECT_ROOT, 'results', 'cascadia', MAG_FILTER)
...
run_ids = {f.stem for f in Path(RUN_DIR).glob('*.run')}
large_event_ids = sorted(
    eid for eid in catalog_df.loc[catalog_df['usgs_mag'] >= MIN_MAG, 'event_id']
    if eid in run_ids
)
...
cache_paths['KDE_Seismicity'] = os.path.join(SeismicPrior.data_dir, 'kde_seismicity_cascadia.tt3')
...
event_ids = sorted(large_event_ids, key=lambda eid: run_trigger_time(eid, RUN_DIR))
