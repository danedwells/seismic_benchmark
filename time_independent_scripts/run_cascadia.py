...
# Override MTJ_EVENT_ID with a specific event_id (ANSS str id) to pin a particular event.

MTJ_EVENT_ID   = 'nc73821036'   # 2022-12-20 M6.4 Ferndale mainshock; None = auto-select from MTJ region
    ...
    if MTJ_EVENT_ID is not None:
        ...
        if ref_lat is not None and ref_lon is not None:
            ...
            for prior_name, (t_out, out_df, actual_v, sp, pcache) in prior_results.items():
                ...
                usgs_contf = 100 * posterior_confidence_level(out_df, ref_lat, ref_lon)
                print(f'  {prior_name}:')
