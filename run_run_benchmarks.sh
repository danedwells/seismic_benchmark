#!/usr/bin/env bash
# run_run_benchmarks.sh — run time_independent and/or time_dependent run_benchmarks.py,
...
#
# Examples:
#   ./run_run_benchmarks.sh --sigma-s "1.0 1.5 2.0"
#   ./run_run_benchmarks.sh --which td --case-study Ridgecrest --alpha "0.1 0.5 1.0"
#   ./run_run_benchmarks.sh --sigma-s "1.0 2.0" --max-trigs "10 15"
...
run_one() {
    ...
    echo "=== sigma_s=${sigma:-default} edt_sigma_s=${edt:-default} dtt_weight=${dtt:-default} max_trigs=${mt:-default} alpha=${alpha:-default} ==="

    if [[ "$WHICH" == "both" || "$WHICH" == "ti" ]]; then
        echo "--- time_independent_scripts/run_benchmarks.py ---"
        python "$SCRIPT_DIR/time_independent_scripts/run_benchmarks.py"
    fi
    if [[ "$WHICH" == "both" || "$WHICH" == "td" ]]; then
        echo "--- time_dependent_scripts/run_benchmarks.py ---"
        python "$SCRIPT_DIR/time_dependent_scripts/run_benchmarks.py"
    fi
}
